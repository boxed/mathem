...
public class Product: Codable {
    ...
    let id: String
    let words: [String]
    init(_ p: ProductJson) {
        self.name = p.name.lowercased()
        self.price = p.price
        self.id = p.id
        self.words = self.name.components(separatedBy:" ")
    }
}

public class ProductJson: Codable {
    let name: String
    let price: Double
    let id: String
}

let data = try Data(contentsOf: URL(fileURLWithPath: "products.json"), options: .mappedIfSafe)
...
let key_distance = memoize {
    (_ c: Character, _ c2: Character) in
    _key_distance(c, c2)
}
